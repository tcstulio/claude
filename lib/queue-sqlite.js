...
class MessageQueueSQLite extends EventEmitter {
  constructor(options = {}) {
    super();
    this._storage = options.storage; // Storage instance (required)
    this._maxRetries = options.maxRetries || DEFAULT_MAX_RETRIES;
    this._timer = null;
    this._sendFn = options.sendFn || null;

    if (!this._storage) throw new Error('MessageQueueSQLite requer uma instância de Storage');
  }

  get pending() { return this._storage.getMessageStats().pending; }
  get delivered() { return this._storage.getMessageStats().delivered; }
  get failed() { return this._storage.getMessageStats().failed; }
