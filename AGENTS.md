# Tulipa — Instruções para OpenAI Codex / Agents

> Este arquivo é lido automaticamente pelo OpenAI Codex como contexto de sessão.
...
- **Runtime:** Node.js >= 22
- **Testes:** Vitest (`npm test`) — 301 testes, 22 test files
- **CI:** GitHub Actions matrix (Ubuntu, Windows, macOS x Node 22, 24)
- **Estrutura:** fonte em `lib-ts/`, testes em `test/`
- **Owner:** Tulio Silva
...
- `GET /api/mesh/peers` — Peers conhecidos
- `POST /api/mesh/prompt/:nodeId` — Enviar prompt a peer (Claude P2P)
- `POST /api/network/logs` — Logs federados (todas as máquinas)
- `POST /api/network/query` — Busca por skill na rede
- `GET /api/ledger/balance` — Saldo do nó

Documentação completa dos 100+ endpoints em `README.md`.
